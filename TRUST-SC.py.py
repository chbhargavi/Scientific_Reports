...
clustering_running_time = (
    clustering_end_time - clustering_start_time
) * 1000
for idx, cl in enumerate(clusters):
    if len(cl) == 0:
        print(f"Cluster {idx+1} is empty")
...
cluster_quality_start_time = time.time()
print("\n================ CLUSTER-WISE WINNERS =================\n")
cluster_winners = {}
overall_total_net_utility = 0
for cluster_id, cluster_execs in cluster_executors.items():
    ...
    while remaining_executors:
        ...
        for v in voters:
            ...
            # allocated index of winner
            allocated_index = preferences[v].index(winner)

            # first preference index = 0
            first_index = 0

            # Utility = allocated index - first index
            utility = allocated_index - first_index

            utility_preferences[v] = utility
            total_net_utility += utility

        print("Utility of task executors preferences:", utility_preferences)
        print("Net utility of task executors of this iteration:", total_net_utility)
        cluster_total_net_utility += total_net_utility
        ...
        iteration += 1
    print(f"Total Net Utility of this Cluster {cluster_id}: {cluster_total_net_utility}")
    overall_total_net_utility += cluster_total_net_utility
...
for c, w in cluster_winners.items():
    ...
    pass
print(f"\nOverall Total Net Utility of All Clusters: {overall_total_net_utility}")
cluster_quality_end_time = time.time()

cluster_quality_running_time = (
    cluster_quality_end_time - cluster_quality_start_time
) * 1000

cluster_plus_quality_time = (
    clustering_running_time +
    cluster_quality_running_time
)

quality_preference_time = cluster_quality_running_time

# =====================================================
# ===== APPLY ALL MECHANISMS FOR EACH CLUSTER =========
# =====================================================

total_requesters = int(input("\nEnter total number of requesters: "))
num_tasks = int(input("Enter number of tasks: "))

TASKS = [f"T{i}" for i in range(1, num_tasks + 1)]

all_requesters = [f"R{i}" for i in range(1, total_requesters + 1)]
random.shuffle(all_requesters)

# ---------- DISTRIBUTE REQUESTERS ----------
cluster_requesters = {}
cluster_ids = list(cluster_winners.keys())

split_size = len(all_requesters) // len(cluster_ids)
extra = len(all_requesters) % len(cluster_ids)

start = 0

for i, cid in enumerate(cluster_ids):

    end = start + split_size + (1 if i < extra else 0)

    cluster_requesters[cid] = all_requesters[start:end]
...
for cid, reqs in cluster_requesters.items():
    print(f"Cluster {cid}: {reqs}")

# =====================================================
# ========== GLOBAL TRACKERS FOR MCAFEE ===============
# =====================================================

overall_mcafee_req_util = 0
overall_mcafee_exe_util = 0
overall_mcafee_req_payment = 0
overall_mcafee_exe_payment = 0
overall_mcafee_time = 0


# =====================================================
# ========== GLOBAL TRACKERS FOR MUDA =================
# =====================================================

overall_muda_req_util = 0
overall_muda_exe_util = 0
overall_muda_req_payment = 0
overall_muda_exe_payment = 0
overall_muda_time = 0


# =====================================================
# ========== GLOBAL TRACKERS FOR TMUDA ================
# =====================================================

overall_tmuda_req_util = 0
overall_tmuda_exe_util = 0
overall_tmuda_req_payment = 0
overall_tmuda_exe_payment = 0
overall_tmuda_time = 0


# =====================================================
# ========== GLOBAL TRACKERS FOR PPM ==================
# =====================================================

overall_ppm_req_util = 0
overall_ppm_exe_util = 0
overall_ppm_req_payment = 0
overall_ppm_exe_payment = 0
overall_ppm_time = 0
# =====================================================
# ===== RUN EACH CLUSTER SEPARATELY ===================
# =====================================================
allocation_start_time = time.time()
for cluster_id, winners in cluster_winners.items():
    # =====================================================
    # ===== CLUSTER OVERALL TRACKERS ======================
    # =====================================================

    cluster_req_utility_total = 0
    cluster_exe_utility_total = 0

    cluster_req_payments_total = 0
    cluster_exe_payments_total = 0

    cluster_execution_time_total = 0

    print("\n\n================================================")
    print(f"RUNNING ALL MECHANISMS FOR CLUSTER {cluster_id}")
    print(f"Cluster Winners: {winners}")
    print(f"Cluster {cluster_id}: {cluster_requesters[cluster_id]}")
    print(f"Enter number of tasks: {num_tasks}")
    print("================================================")

    requesters = cluster_requesters[cluster_id]

    final_executors = winners
    if len(final_executors) < 2:
            print("Too few executors in cluster — skipping cluster.")
            continue
    ...
    def execute_mcafee(tasks, task_to_r, task_to_e, req_val, exe_cost):
        ...
        print("Total requester utility(price-based):", round(total_req_utility,2))
        print("Total executor utility (price-based):", round(total_exe_utility,2))
        ...
        return (
            total_req_utility,
            total_exe_utility,
            total_payment_by_requesters,
            total_payment_to_executors,
    ...
    cluster_req_utility_total += mcafee_req_util
    cluster_exe_utility_total += mcafee_exe_util

    cluster_req_payments_total += mcafee_req_payment
    cluster_exe_payments_total += mcafee_exe_payment

    cluster_execution_time_total += mcafee_time 
    
    overall_mcafee_req_util += mcafee_req_util
    overall_mcafee_exe_util += mcafee_exe_util

    overall_mcafee_req_payment += mcafee_req_payment
    overall_mcafee_exe_payment += mcafee_exe_payment

    overall_mcafee_time += mcafee_time  
    #----------------MUDA---------------------------#
    ...
    if len(LSCZ["requesters"]) < 1 or len(LSCZ["executors"]) < 1:
        print("Left market too thin — skipping MUDA for this cluster.")
        continue

    if len(RSCZ["executors"]) < 1 or len(RSCZ["requesters"]) < 1:
        print("Right market too thin — skipping MUDA for this cluster.")
        continue
    ...
    def execute_MUDA(zone, price, tasks, task_to_r, task_to_e, req_vals, exe_costs):
        ...
        print("Total Requester utilities(price-based):", total_req_utility)
        print("Total Executor utilities(price-based) :", total_exe_utility)
    ...
    cluster_req_utility_total += muda_req_util
    cluster_exe_utility_total += muda_exe_util

    cluster_req_payments_total += muda_req_payment
    cluster_exe_payments_total += muda_exe_payment

    cluster_execution_time_total += muda_time
    overall_muda_req_util += muda_req_util
    overall_muda_exe_util += muda_exe_util

    overall_muda_req_payment += muda_req_payment
    overall_muda_exe_payment += muda_exe_payment

    overall_muda_time += muda_time
    ...
    print("Total Utility of task requesters in LSCZ (price-based):", total_req_util_LSCZ)
    print("Total Utility of task requesters in RSCZ (price-based):", total_req_util_RSCZ)

    print("Total Utility of task executors in LSCZ(price-based) :", total_exe_util_LSCZ)
    print("Total Utility of task executors in RSCZ(price-based) :", total_exe_util_RSCZ)
    ...
    print("Total payment made by requesters (RSCZ) :", total_payment_by_requesters_RSCZ)
    print(f" Execution time : {elapsed_ms:.3f} milliseconds")
    
    cluster_req_utility_total += total_req_util_LSCZ

    cluster_exe_utility_total += total_exe_util_LSCZ

    cluster_req_payments_total += total_payment_by_requesters_LSCZ

    cluster_exe_payments_total += total_payment_to_executors_LSCZ

    cluster_execution_time_total += elapsed_ms
    
    overall_tmuda_req_util += total_req_util_LSCZ

    overall_tmuda_exe_util += total_exe_util_LSCZ

    overall_tmuda_req_payment += total_payment_by_requesters_LSCZ

    overall_tmuda_exe_payment += total_payment_to_executors_LSCZ
    overall_tmuda_time += elapsed_ms
    ...
    #print("Task units completed:", completed)
    print("Total requester utility (price-based)       :", total_requester_utility)
    print("Total executor utility(price-based)         :", total_executor_utility)
    print("Total payment made to executors :", total_payment_to_executors)
    print("Total payment made by requesters :", total_payment_by_requesters)
    ...
    print(f" Execution time : {elapsed_ms:.3f} milliseconds")
    cluster_req_utility_total += total_requester_utility
    cluster_exe_utility_total += total_executor_utility

    cluster_req_payments_total += total_payment_by_requesters
    cluster_exe_payments_total += total_payment_to_executors

    cluster_execution_time_total += elapsed_ms
    overall_ppm_req_util += total_requester_utility
    overall_ppm_exe_util += total_executor_utility

    overall_ppm_req_payment += total_payment_by_requesters
    overall_ppm_exe_payment += total_payment_to_executors

    overall_ppm_time += elapsed_ms

    print("\n================================================")
    print(f"COMPLETED CLUSTER {cluster_id}")
    print("================================================")
    print("\n================================================")
    print(f"FINAL OVERALL SUMMARY FOR CLUSTER {cluster_id}")
    print("================================================")

    print("Total Requester Utility of Cluster :",
        cluster_req_utility_total)

    print("Total Executor Utility of Cluster  :",
        cluster_exe_utility_total)

    print("Total Payments by Requesters of Cluster :",
        cluster_req_payments_total)

    print("Total Payments to Executors of Cluster  :",
        cluster_exe_payments_total)

    print(f"Total Execution Time of Cluster : "
        f"{cluster_execution_time_total:.3f} milliseconds")
allocation_end_time = time.time()

allocation_running_time = (
    allocation_end_time - allocation_start_time
) * 1000

total_algorithm_time = (
    clustering_running_time
    + quality_preference_time
    + allocation_running_time
)
# =====================================================
# ============ FINAL SUMMARY OF ALL CLUSTERS ==========
# =====================================================

print("\n\n================================================")
print("FINAL OVERALL SUMMARY OF ALL MECHANISMS")
print("================================================")

print(f"\nOverall Total Net Utility of All Clusters: {overall_total_net_utility}")
# ================= MCAFEE =================
print("\n=========== OVERALL MCAFEE SUMMARY ===========")

print("Overall Requester Utility (price-based):",
      overall_mcafee_req_util)

print("Overall Executor Utility (price-based):",
      overall_mcafee_exe_util)

# print("Overall Payments by Requesters:",
#       overall_mcafee_req_payment)

print("Overall Payments to Executors:",
      overall_mcafee_exe_payment)

# print(f"Overall Execution Time: "
#       f"{overall_mcafee_time:.3f} ms")
# ================= MUDA =================
print("\n=========== OVERALL MUDA SUMMARY ===========")

print("Overall Requester Utility (price-based):",
      overall_muda_req_util)

print("Overall Executor Utility (price-based):",
      overall_muda_exe_util)

# print("Overall Payments by Requesters:",
#       overall_muda_req_payment)

print("Overall Payments to Executors:",
      overall_muda_exe_payment)

# print(f"Overall Execution Time: "
#       f"{overall_muda_time:.3f} ms")
# ================= TMUDA =================
print("\n=========== OVERALL TMUDA SUMMARY ===========")
print("Overall Requester Utility (price-based):",
      overall_tmuda_req_util)

print("Overall Executor Utility (price-based):",
      overall_tmuda_exe_util)

# print("Overall Payments by Requesters:",
#       overall_tmuda_req_payment)

print("Overall Payments to Executors:",
      overall_tmuda_exe_payment)

# print(f"Overall Execution Time: "
#       f"{overall_tmuda_time:.3f} ms")
...
print("Overall Requester Utility (price-based):",
      overall_ppm_req_util)

print("Overall Executor Utility (price-based):",
      overall_ppm_exe_util)

# print("Overall Payments by Requesters:",
#       overall_ppm_req_payment)

print("Overall Payments to Executors:",
      overall_ppm_exe_payment)

# print(f"Overall Execution Time: "
#       f"{overall_ppm_time:.3f} ms")
      
print("\n")
print("================================================")
print("FINAL RUNNING TIME SUMMARY")
print("================================================")

print(f"1. Clustering Running Time (K-Means Only) : "
      f"{clustering_running_time:.3f} ms")

print(f"2. Clustering + Quality Preference + Winner Selection Time : "
      f"{cluster_plus_quality_time:.3f} ms")

print(f"3. Overall Running Time : "
      f"{total_algorithm_time:.3f} ms")

print(f"4. McAfee Running Time : "
      f"{overall_mcafee_time:.3f} ms")

print(f"5. MUDA Running Time : "
      f"{overall_muda_time:.3f} ms")

print(f"6. TMUDA Running Time : "
      f"{overall_tmuda_time:.3f} ms")

print(f"7. PPM Running Time : "
      f"{overall_ppm_time:.3f} ms")

print(f"8. Overall McAfee Running Time "
      f"(Clustering + QTESM + McAfee) : "
      f"{(cluster_plus_quality_time + overall_mcafee_time):.3f} ms")

print(f"9. Overall MUDA Running Time "
      f"(Clustering + QTESM + MUDA) : "
      f"{(cluster_plus_quality_time + overall_muda_time):.3f} ms")

print(f"10. Overall TMUDA Running Time "
      f"(Clustering + QTESM + TMUDA) : "
      f"{(cluster_plus_quality_time + overall_tmuda_time):.3f} ms")

print(f"11. Overall PPM Running Time "
      f"(Clustering + QTESM + PPM) : "
      f"{(cluster_plus_quality_time + overall_ppm_time):.3f} ms")

print(f"12. Quality Preference (QTESM) Running Time : "
      f"{quality_preference_time:.3f} ms")